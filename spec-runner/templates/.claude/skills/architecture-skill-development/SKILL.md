...
### code-common.md / code-backend.md / code-frontend.md の書き換え

`architecture.yaml` の `scope` を確認する:
- `scope: backend` のみ → `code-common.md` は不要。`code-backend.md` に全ルールを記述
- `scope: frontend` のみ → `code-common.md` は不要。`code-frontend.md` に全ルールを記述
- `scope: all`（フロント・バック両方） → **`code-common.md` に共通ルールを集約し、`code-backend.md` / `code-frontend.md` は言語固有ルールのみ記述**

#### フロント・バック両方ある場合（`scope: all`）

`code-common.md` は既にテンプレートに含まれている。以下を確認:
1. `code-common.md` の内容が適切か確認（コメント・テスト ID・後方互換禁止・エラーハンドリング）
2. `code-backend.md` と `code-frontend.md` の冒頭に `code-common.md` への参照があることを確認
3. プロジェクト固有のコメント規約をユーザーと合意（必要に応じて `code-common.md` を更新）
