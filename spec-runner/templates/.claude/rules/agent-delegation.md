...
| エラー・技術調査 | サブエージェント（WebSearch） | 解決策不明時。`.spec-runner/references/resources.md` の URL 起点 |
...
1. 目的とゴール（冒頭1文）
2. 仕様は抽出ブロックで埋め込む: `node .spec-runner/scripts/extract.js "{node_id}" --blocks ...` の出力を `## 仕様` として埋め込む（ブロックは design-docs.md の抽出表）。フェンスなし設計書のみパスを渡す
