...
5. 共通ハーネスを別プロジェクトから移植した直後、または大規模リネーム後に、プロジェクト固有化と重複排除を仕上げたい
...
3. **`.claude/` と `.github/` を必ず対で更新する**
   - skill を変更した場合: `.claude/skills/{name}/SKILL.md` と `.github/skills/{name}/SKILL.md` の両方を更新する
   - rule を変更した場合: `.claude/rules/{name}.md` と `.github/instructions/{name}.instructions.md` の両方を更新する
   - agent を変更した場合: `.claude/agents/{name}.md` と `.github/agents/{name}.agent.md` の両方を更新する
   - 片方だけ更新して終わらない。`harness-format.md` の「共通原則 > integrations」も参照
4. references や templates を参照している場合、必要な範囲だけ更新する

## Phase 4: 影響範囲の反映確認

`references/harness-format.md` の「プロジェクト固有化チェック」「重複排除チェック」を当てる。加えて以下を確認する。
