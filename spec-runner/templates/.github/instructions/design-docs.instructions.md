...
対象: 実装と1対1対応する詳細設計（UC・ドメイン・画面・コンポーネント・外部サービス）。frontmatter 直後に ```yaml フェンスを1つだけ置き、本文は全てその中に書く。フェンス外の記述禁止（抽出・lint から漏れる）。
...
| ブロック | 必須 | 内容 |
|---|---|---|
| `概要` | 必須 | `title` / `purpose`（目的1文。How 禁止） |
...
- 詳細設計に図を書かない。シーケンス -> `フロー`、状態遷移 -> `状態.遷移`、集約 -> `状態` の構造データが正本（図は二重記載 -> 乖離）
- 俯瞰図（システム構成・画面遷移・ドメイン概念・状態遷移）は概要設計に Mermaid で書く
...
**lint（仕様内整合）**: 必須ブロック欠落・順序違反・未知ブロック / 死に定数 / 値の直書き / 例外カバレッジ / 公開IF エラー参照の実在 / T-XX 形式・重複 / 未参照 input

**drift（仕様⇔実装。maps_to 先コードを文字列突合）**: 定数（名前 or 値）/ 公開IF の path・method（Router 未配線検出）/ 例外型 / input 名 / T-XX 双方向（仕様にあるのにテストなし・テストにあるのに仕様なし）

drift が警告止まりの理由: config・env 経由の正当な間接参照があるため。意図的な間接参照なら定数コメントに経路を書く。
